...
export const initCreatures = ($stage) => {
  const stageSize = {
    w: $stage.clientWidth,
  ...
  // Función para seleccionar color con probabilidades ponderadas
  ...
  // Spawn multiple initial creatures
  ...
  // Spawn permanent sleeping creature in right corner (never wakes up)
  const createPermanentSleepingCreature = () => {
    const permanentColor = '#ffa664'; // Medium orange
    const permanentSize = 130;
    const permanentLeg = permanentSize * 0.4;
    const permanentX = stageSize.w - 160; // Right side (adjusted for larger size)
    const permanentY = stageSize.h - (permanentSize / 2) + 10; // Half buried in the ground
    ...
    // Position it in the right corner, half buried
    gsap.set($permanentCreature, {
      x: permanentX,
      y: permanentY,
    });

    // Keep it in idle/sleeping state forever with breathing animation
    const breathe = () => {
    ...
    // Spawn z's periodically (larger and more noticeable)
    const spawnZs = () => {
      if (Math.random() < 0.5) { // Increased probability
        spawnParticle($stage, {
          ...
          scale: gsap.utils.random(1.2, 1.8), // Much larger z's
    ...
    // Make creature clickable - link to team page
    const $dragger = $permanentGroup.querySelector('.dragger');
    $dragger.style.cursor = 'pointer';
    $dragger.style.pointerEvents = 'all'; // Enable only on dragger and body

    $permanentBody.style.cursor = 'pointer';
    $permanentBody.style.pointerEvents = 'all'; // Enable only on dragger and body

    // Add click handler to both dragger and body for better hit area
    const handleClick = (e) => {
      e.preventDefault();
      e.stopPropagation();
      window.open('https://mandoc2-editorial.netlify.app/about', '_blank');
    };

    $dragger.addEventListener('click', handleClick);
    $permanentBody.addEventListener('click', handleClick);
