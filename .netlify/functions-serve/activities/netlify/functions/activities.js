...
function getDB() {
  if (!sql) {
    ...
    if (!connectionString) {
      throw new Error("DATABASE_URL no est\xE1 configurada en las variables de entorno");
    }
    sql = cs(connectionString);
  }
  return sql;
}
var corsHeaders = {
...
async function handler(event) {
  ...
  try {
    ...
    if (httpMethod === "GET") {
      ...
      const activities = await sql2(query, values);
    ...
    return error("M\xE9todo no permitido", 405);
  } catch (err) {
